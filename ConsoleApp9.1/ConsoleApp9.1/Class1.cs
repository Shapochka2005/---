...
namespace ConsoleApp9._1
{
    internal class Program
    {
        ...
        private static void Draw()
        {
            ...
            {
                new string('O', _size.Width - 2).Write(1, i, ConsoleColor.Black, ConsoleColor.Black);
            }

            _head.Write("0", ConsoleColor.Yellow, ConsoleColor.Black);
            _tail.Write("o");
            _fruit.Write("Яблоко", ConsoleColor.Cyan, ConsoleColor.Black);
        ...
        private static void Logic()
        {
            ...
            {
                if (_head.Y - 1 < 1) { _gameOver = true; }
                else { _head.Y--; }
            }
            else if (_direction == Direction.Down)
            {
                if (_head.Y + 1 > _size.Height - 2) { _gameOver = true; }
                else { _head.Y++; }
            }
            else if (_direction == Direction.Left)
            {
                if (_head.X - 1 < 1) { _gameOver = true; }
                else { _head.X--; }
            }
            else if (_direction == Direction.Right)
            {
                if (_head.X + 1 > _size.Width - 2) { _gameOver = true; }
                else { _head.X++; }
            }
        }
        ...
        {
            $"КОНЕЦ ИГРЫ".Write(_size.Width + 3, 3, ConsoleColor.Red, ConsoleColor.White);
            $"Нажмите любую кнопку для продожения".Write(_size.Width + 3, 4, ConsoleColor.Black, ConsoleColor.Gray);
